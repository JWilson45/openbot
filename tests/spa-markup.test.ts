...
describe("desk SPA markup", () => {
  test("ships skip link, noscript sign-in, live region, and language", async () => {
    ...
    expect(js).not.toContain("gwSelected");
    expect(js).toContain("state.view !== 'human' && state.view !== 'group'");
    expect(js).toContain("{ agentId:state.bot.id }");
    expect(js).toContain("{ threadId:state.thread.id }");
    ...
    expect(js).toContain("Canonical AG-UI activity");
    expect(js).not.toContain('id="live-raw"');
    expect(js).not.toContain('id="live-human"');
    ...
    expect(js).not.toContain("open-gateway");
    expect(js).not.toContain("openbot/Gateway");
    expect(js).toContain("A2A");
    expect(js).toContain("/.well-known/agent-card.json");
    expect(js).toContain("/a2a/v1");
    expect(js).not.toContain("fed-on");
    expect(js).not.toContain("fed-off");
    expect(js).not.toContain("Federation");
    expect(js).not.toContain("federationEnabled");
    expect(js).not.toContain("/fed/v1/info");
    expect(js).toContain("/v1/threads?kind=group");
    expect(js).toContain("new-group");
    expect(js).toContain("data-group");
    expect(js).toContain("m.origin !== 'prompt'");
    expect(js).toContain("m.origin !== 'calendar'");
    expect(js).toContain("turnIds");
    expect(js).toContain("_turnIds");
    expect(js).not.toContain("'message.created'");
    expect(js).not.toContain("'turn.updated'");
    ...
    expect(js).toContain("They are stored only in this browser.");
    ...
    expect(js).not.toContain("/v1/org/peers");
    expect(js).not.toContain("/v1/org/peers/from-info");
    expect(js).not.toContain("/v1/org/inbox");
    expect(js).not.toContain('id="peer-list"');
    expect(js).not.toContain('id="peer-from-info"');
    expect(js).not.toContain('id="solicit-list"');
    expect(js).not.toContain("Copy pubkey");
    expect(js).not.toContain("tried to send mail");
    expect(js).toContain("navigator.clipboard.writeText");
    expect(js).not.toContain("thread_bridges");
    expect(js).not.toContain("peerPreview.baseUrl");
