...
describe("openai-compatible api", () => {
  ...
  test("GET /v1/models excludes the Gateway sidecar by both name and UUID", async () => {
    process.env.OPENBOT_ACP_COMMAND = fakeAgentCommand();
    const { ctx, server, origin } = startTestServer({ home: tempHome() });
    try {
      ...
      const gateway = ctx.db.get<{ id: string; name: string }>(
        "SELECT id, name FROM bots WHERE account_id = ? AND IFNULL(role, 'desk') = 'gateway' AND status = 'active'",
        [session.accountId],
      );
      ...
      expect(ids).toContain("openbot/Ada");
      expect(ids).not.toContain(`openbot/${gateway!.name}`);
