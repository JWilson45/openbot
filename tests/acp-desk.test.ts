...
describe("ACP session on the desk (fake agent)", () => {
  test("user message runs with cwd = desk; native file work; tab close does not cancel", async () => {
    const home = tempHome();
    ...
    const posted = await fetch(`${origin}/v1/threads/${thread.thread.id}/messages`, {
      method: "POST",
      headers,
      body: JSON.stringify({ body: `[[write:${filename}]] [[send:wrote file]]` }),
    });
    ...
    const deskFile = join(home, "desk", filename);
    expect(existsSync(deskFile)).toBe(true);
    const contents = readFileSync(deskFile, "utf8");
    expect(contents).toContain(join(home, "desk"));
    expect(messages.some((m) => m.origin === "send_message" && m.body.includes("wrote"))).toBe(true);
